 ...
 */
export function traverse(parent: Group, nodes: Array<OutlinerNode>, accu: Array<VS_Element>, offset: [number,number,number]) {
    for (const node of nodes) {
        if (node instanceof Group) {
            process_group(parent, node, accu, traverse, offset);
